...
class BullMQWorker:
    ...
    def get_next_job(self):
        ...
        BullMQ uses a complex mechanism with scripts, but we'll use a simpler approach:
        1. Get the first job ID from the wait list
        2. If a job ID is found, fetch the job data and return it
        ...
        try:
            ...
            # In a real implementation, we should use RPOPLPUSH to move the job to active atomically
    ...
    def process_job(self, job_data):
        ...
        Args:
            job_data (dict): The job data to process
        ...
        logger.info(f"Processing job {job_id} ({name})")
        logger.info(f"Job payload: {json.dumps(data)}")
