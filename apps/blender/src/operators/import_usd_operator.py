...
class ImportUSDOperator(Operator, ImportHelper):
    ...
    def draw(self, context):
        ...
        layout.prop(scene, "import_usd_operator_import_materials", text="Import Materials")
...
def register():
    Scene.import_usd_operator_import_cameras = BoolProperty(name="Import Cameras", default=True)
    Scene.import_usd_operator_import_curves = BoolProperty(name="Import Curves", default=True)
    Scene.import_usd_operator_import_lights = BoolProperty(name="Import Lights", default=True)
    Scene.import_usd_operator_import_materials = BoolProperty(name="Import Materials", default=True)
    Scene.import_usd_operator_import_meshes = BoolProperty(name="Import Meshes", default=True)
    Scene.import_usd_operator_scale = FloatProperty(name="Scale", default=1.0, min=0.0001, soft_max=1000.0)
